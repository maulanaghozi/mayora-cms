  ...
  const handleChangeStatus = (user_id, status) => {
    ...
    http(params).then(res => {
      if(res && res.code === "success") {
        ...
        alert.error(`ubah status recruiter ke ${status} gagal`)
      }
    })
  }
  ...
  return (
    <React.Fragment>
      <div className={classNames(
        ...
        <Link 
          to={`/user/recruiter/${props.row.user_id}`}
        >
          <ViewIcon />
        </Link>

        <Link 
          to={`/user/recruiter/${props.row.user_id}/casting`}
        >
          <CastingIcon />
        </Link>
      ...
      {isOpenWarning && 
        <WarningModal
          ...
          content={"Anda yakin block Recruiter ini?"}
        />
      }
    </React.Fragment>
  )
