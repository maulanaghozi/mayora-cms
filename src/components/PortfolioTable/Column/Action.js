...
export default function Action(props) {
    ...
    const deletePortfolio = () => {
        http({
            method: 'DELETE',
            path: 'promotion/portfolio/' + props.row.id
        }).then(result => {
            if (result && result.code === 'success') {
                ...
                alert.error('failed to delete portfolio')
            }
        })
    }
    ...
    return (
        <div className={classNames(style.table_cell, style.column_action)}>
            ...
            <Link to={`/home-content/portfolio/edit/${props.row.id}`}><EditIcon /></Link>
            <div style={{ position: 'relative' }}>
                ...
                {isOpen &&
                    <div ref={boxRef} className={classNames(style.more_box, {[style.reverse]: props.index > props.totalRow - 3})}>
                        ...
                        <div onClick={deletePortfolio} className={classNames(style.more_text, style.red)}>Delete</div>
                    </div>
                }
            </div>
            {
                ...
                <ContentViewer
                    index={props.index}
                    content={props.data.map(portfolio => portfolio.video_url)}
